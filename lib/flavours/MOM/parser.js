  ...
  , UI
    ...
    engine = new Parser(
      ...
      , new Operator('UI', false, -Infinity, 0, Infinity, (function(){
            ...
            // It's used it to set `uiOperator.inject` in place and thus
            // the definition of this operator needs not be move to somewhere
            // else. Also, the namespace is a bit less polluted.
            function uiOperator (getAPI /*, args, ...*/) {
                var i, l
                  , args = []
                  , property = getAPI.getCurrentProperty()
                  , rule = getAPI.getRuleForProperty(property)
                  ;
                for(i=1,l=arguments.length;i<l;i++)
                    args.push(arguments[i]);
                return new UI(property, rule, args);
            }
            uiOperator.inject = '*unboxed+getAPI*';
            return uiOperator;
        })()
        )
        /**
