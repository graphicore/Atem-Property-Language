define([
    'Atem-Property-Language/errors'
  , 'Atem-Property-Language/parsing/Parser'
  , 'Atem-Property-Language/parsing/OperatorToken'
  , 'Atem-Property-Language/parsing/PostfixExpression'
  , 'Atem-Property-Language/parsing/NumberToken'
], function(
    errors
  , Parser
  , Operator
  , PostfixExpression
  , NumberToken
) {
    "use strict";

    var PropertyLanguageError = errors.PropertyLanguage
      , parser
      ;

    function SimpleMathExpression(postfixStack, originalCode) {
        this._code = originalCode;
        PostfixExpression.call(this, postfixStack);
    }
    var _p = SimpleMathExpression.prototype = Object.create(PostfixExpression.prototype);
    _p.constructor = SimpleMathExpression;

    _p._check = function(stack) {
        var i, l, invalid=[];
        for(i=0,l=stack.length;i<l;i++) {
            // whitelisting allowed tokens
            if(!(stack[i] instanceof NumberToken || stack[i] instanceof Operator))
                invalid.push(stack[i]);
        }
        if(invalid.length)
            throw new PropertyLanguageError('The math expression contains invalid tokens: ' + invalid.join(', '));
        return PostfixExpression.prototype._check.call(this, stack);
    };

    _p.toString = function() {
        return this._code;
    };

    /**
     * This method is applied in PostfixExpression.execute, with the result
     * of the postfix stack execution.
     *
     * OperatorToken._convertTokenToValue does something similar.
     */
    _p._finalizeMethod = function(result, getAPI) {
        /* jshint unused:vars */
        if(typeof result != 'number')
            throw new PropertyLanguageError('The math expression did not yield in a number. Got instead: ' + result);
        return result;
    };

    /**
     * This defines the operators that are usable in simple math expressions
     * I.e. this is a calculator.
     *
     * see the reference of new Operator for a description of its arguments.
     *
     * usage: engine.parse(CPSParameterValueString) returns a Stack
     *        engine.parse('1 + 2 * 3').execute() returns 7
     */
    parser = new Parser(
        /**
         * When a value is negated using the minus sign, this operator is
         * inserted instead of the minus sign. It can also be used directly.
         *
         * The parser should detect cases where the minus sign is not a
         * subtraction, but a negation:
         *
         * -5 => negate 5
         * -(5 + name) => negate (5 + name)
         * 5 + -name => 5 + negate name
         * 5 + - name => 5 + negate name
         * name * - 5 => name * negate name
         *
         */
        new Operator('negate', false, 60, 0, 1, [
            // 'number' as an argument is not needed nor happening
            // because something like -123 will be parsed as a negative
            // number directly. This is because "Vector 12 -8" would
            // otherwise be tokenized as "Vector 12 subtract 8", because
            // we have no other indication of splitting.
            // the operator is left in place, so this: --123 could be done
            // and would result in `negate -123`
            ['number', function(a){ return -a; }]
        ])
          /**
           * add
           */
      , new Operator('+', true, 10, 1, 1, [
            ['number' , 'number', function(a, b){ return a + b; }]
        ])
        /**
         * subtract
         */
      , new Operator('-', true, 10, 1, 1, [
            ['number' , 'number', function(a, b){ return a - b; }]
        ])
        /**
         * multiply
         */
      , new Operator('*', true, 20, 1, 1, [
           ['number' , 'number', function(a, b){ return a * b; }]
        ])
        /**
         * divide
         */
      , new Operator('/', true, 20, 1, 1, [
            ['number' , 'number', function(a, b){ return a / b; }]
        ])
        /**
         * pow
         *
         * FIXME: associativity must be set to 1 which means right-associative.
         */
      , new Operator('^', true, 30, 1, 1, [
            ['number' , 'number', function(a, b){ return Math.pow(a, b); }]
        ])
      , new Operator('min', true, 40, 0, 2, [
            ['number' , 'number', function(a, b){ return Math.min(a, b); }]
        ])
      , new Operator('max', true, 40, 0, 2, [
            ['number' , 'number', function(a, b){ return Math.max(a, b); }]
      ])
    );

     /**
      * Return a new instance of SimpleMathExpression, for the parsed
      * expression string;
      */
    parser.parse = function(expression) {
        var ast = parser.stringToAST(expression)
          , postfix = parser.astToPostfix(ast)
          ;
        return new SimpleMathExpression(postfix, expression);
    };

    parser.setNegateOperator('-', 'negate');
    return parser;
});
