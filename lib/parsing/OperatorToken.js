define([
  ...
  , NameToken
     ...
     *
     * preConsumes,
     * postConsumes: How many tokens this operator consumes
     ...
     *
     * FIXME:
     * We can't define this yet in the constructor, however, we need it at
     * least for the ^ operator which represents exponentiation.
     */
    Object.defineProperty(_p, 'associativity', {
        get: function(){ return -1; }
    });

    _p.isLeftAssociative = function() {
        return this.associativity === -1;
    };

    _p.isRightAssociative= function() {
        return this.associativity === 1;
    };
     ...
     *
     * - A flag, only as very first element, like *getAPI*.
     *   See the documentation of _parseFlags.
     ...
     * and beginning and ending with a single asterix.
     * Whitespace at the beginning and end of a flag is removed.
     *
     * examples:
     * '*getAPI*'
     * '*getAPI, optional*'
     * '*optional, unboxed+getAPI*'
     *

     *
     * '*getAPI*' this does two things:
     *   * it injects the getAPI function as first argument of method
     *   * it does NOT resolve NameToken arguments using getAPI(nameToken.getValue())
     *     so, specifiying *getAPI* means method will handle NameTokens by itself.
     ...
     *     *getAPI*.
     * '*unboxed+getAPI*' if it is more convenient to have NameToken arguments
     *     resolved, but still a reference to to `getApi`. If this flag and
     *     "getAPI" are present, "getAPI" wins and NameToken are passed to the
     *     method.
     * '*optional*'
     *     Pass the third argument of the call to `this.execute` to the method.
     *     If `getApi` is requested this will be the second argument to the call
     *     to method, otherwise it will be the first.
     *     References: http://atem.graphicore.de/content/Proposals/cps-ui.md.html#the-hack
     *
     */
    _p._parseFlags = function(str) {
        var flags = new Set();
        if((typeof str !== 'string')
                        || str[0] !== '*'
                        || str.slice(-1) !== '*'
                        // This is not a flag argument
                        || str === '*anything*')
            // has no flags argument
            // test with:  flags.has(null)
            flags.add(null);
        else
            str.slice(1,-1)
                        .split(',')
                        .map(function(s){return s.trim();})
                        .forEach(Set.prototype.add, flags);
        return flags;
    };

    _p._getMethodFlags = function(description) {
        if( this._methodsFlags.has(description) )
            return this._methodsFlags.get(description);
        var flags = typeof description === 'function'
                        ? this._parseFlags(description.flags)
                        : this._parseFlags(description[0])
                        ;
        this._methodsFlags.set(description, flags);
        return flags;
    };
    ...
    _p._setMethod = function(description) {
        ...
        // It is an array
        flags = this._getMethodFlags(description);
        expectedLength = this.consumes + (flags.has(null) ? 1 : 2);
        ...
        if(description.length !== expectedLength)
            ...
            throw new ValueError('The last item of an operator definition '
                ...
                + (typeof description[description.length-1])+'"');
                    ...
                    && flags.has('getAPI') // this is boxed
                    && !this._getMethodFlags(this._methods[this._methods.length-1]).has('getAPI'))// the previous is unboxed
    ...
    // helper for compile
    _p._makeLocalBoxedNames = function(body, methods, consumes) {
        var i, l, flags, isBoxed
          , usersOfBoxed = 0
          , args = _makeArgNames(consumes, 'args[', ']')
          , boxedNames
          ;
        for(i=0, l=methods.length;i<l; i++) {
            flags = this._getMethodFlags(methods[i]);
            isBoxed = flags.has('getAPI');
    ...
    _p.compile = function () {
        /*jshint evil:true*/
        var body = ['"use strict";\n']
          , i, j, k, l, ll, description, flags, methodName
        ...
        names = this._makeLocalBoxedNames(body, this._methods, this.consumes);
        args = names.join(', ');
        ...
        for(i=0, l=this._methods.length;i<l; i++) {
            ...
            isBoxed = flags.has('getAPI');
            hasGetAPI = isBoxed || flags.has('unboxed+getAPI');
            injected = [];
            if(hasGetAPI)
                injected.push('getAPI');
            if(flags.has('optional'))
                injected.push('optional');
            ...
            if(typeof description === 'function') {
                ...
                hasMatchAll = true;
                body.push('return ', methodName,'('
                        , injected.join(', '), (injected.length ? ', ' : '')
                        , args
                        , ');');
                break;
            }
            ...
            body.push('if(true');
            // k: start at 1 if the first item is a '*getAPI*' like flags argument
            for(j=0, k=flags.has(null)?0:1,ll=this.consumes;j<ll;k++,j++) {
                type = description[k];
                // always true
        ...
        return new Function('getAPI', 'args', 'optional', body.join(''));
    };
    ...
    _p._findMethod = function(getAPI, argsObj) {
        var index=0, length=this._methods.length, flags, j, k, type, args;
        ...
        for(;index<length; index++) {
            ...
            flags = this._getMethodFlags(this._methods[index]);
            // if it has flags start at the second argument
            j = flags.has(null) ? 0 : 1;

            args = flags.has('getAPI')
                        ? argsObj.original
                        : argsObj.unboxedNameTokens
                        ;
     ...
     * implementation
     *
     * In http://atem.graphicore.de/content/Proposals/cps-ui.md.html#the-hack
     * is a discussion of the initial reason and usage of the `optional`
     * argument.
     */
    _p.execute = function(getAPI, _args, optional) {
        ...
        operator = typeof description === 'function'
                    ? description
                    : description.slice(-1)[0]
                    ;

        args = [];
        if(flags.has('getAPI') || flags.has('unboxed+getAPI'))
            args.push(getAPI);
        if(flags.has('optional'))
            args.push(optional);
