define([
  ...
  , NameToken
    ...
    function astToPostfix(node) {
        var tokens = [node], item, i=0, operation;
        while(i<tokens.length) {
            ...
            if(item instanceof Grouping) {
                ...
                operation = [i, 1];
                Array.prototype.push.apply(operation, item.arguments);
                operation.push(item.operator);
                // change in place
                Array.prototype.splice.apply(tokens, operation);
                // don't increment, visit this index in the next round
                continue;
            }
            i++;
