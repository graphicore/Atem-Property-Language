define([
  ...
  , ASTGrouping
            ...
            if(this._negateOperator
                ...
                for(j=i-1;j>=0;j--) {
                    if(tokens[j] instanceof _VoidToken)
                        continue;
                    if(tokens[j] instanceof OperatorToken
                                // A BracketToken must be an opening one. A closing
                                // BracketToken implies that we want to subtract from
                                // the result of the expression in the bracket.
                            || tokens[j] instanceof BracketToken && tokens[j].opening)
                        break;
    ...
    /**
     * For a discussion of the `keepASTOperations` argument see:
     * http://atem.graphicore.de/content/Proposals/cps-ui.md.html#the-hack
     */
