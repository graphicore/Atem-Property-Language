define([
  ...
  , NameToken
    ...
    function Operation(operatorToken, args, postArguments) {
        ...
        if(postArguments) {
            ...
            // Operation nodes, so that we don't have to know exactly how
    ...
    function Grouping(groupingToken, nodes) {
        Object.defineProperties(this, {
            groupingToken: {
                value: groupingToken
              , enumerable: true
        ...
        for(j=0,l=operatorIndexes.length;j<l;j++) {
            ...
            if(token.preConsumes === Infinity)
                ...
                for(k=0;k<preConsumes;k++) {
                    if(preConsumes > i) {
                        console.log('!', token, preConsumes, i, tokensToString(tokens, true));
            ...
            for(k=0; k<args.length; k++) {
                ...
                if(!isInstance(argument, [_ValueToken, Grouping, Operation, _VoidToken]))
            ...
            tokens.splice(preStart, args.length + 1, new Operation(operator, preArgs, postArgs));
    ...
    _p.infixToAST = function (tokens) {
        ...
        for(i=0,l=tokens.length;i<l;i++) {
            ...
            // it is a bracket
                ...
                finished = new Grouping(currentBracket, nodes);
        ...
        return new Grouping(null, _toAST(result));
    };
