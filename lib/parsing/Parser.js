define([
  ...
  , NameToken
    ...
    //  */
    // function _toString(tokens, asArray) {
    //     var i, result = [], sub;
    //     for(i=0;i<tokens.length;i++) {
    //         if(tokens[i] instanceof Array){
    //             sub = _toString(tokens[i], asArray);
    ...
    function _toPostfix(tokens) {
        var i, j, k, l, token
          , preConsumes, postConsumes, startPre, startPost, endPost, operation
        ...
        for(j=0,l=operatorIndexes.length;j<l;j++) {
            ...
            operation.push( token.preConsumes === Infinity
