  ...
  , NameToken
    ...
    function _flatten(tokens) {
        ...
        while(i<tokens.length) {
            ...
            if(item instanceof Array) {
                // splice arguments 0 and 1
                item = [i, 1];
                Array.prototype.push.apply(item, tokens[i]);
                // change in place
                Array.prototype.splice.apply(tokens, item);
                // don't increment
                continue;
            }
            i++;
    ...
    function _toPostfix(tokens) {
        ...
        for(j=0,l=operatorIndexes.length;j<l;j++) {
            ...
            Array.prototype.push.apply(operation, tokens.slice(startPost, endPost));

            // check if everything looks alright
            for(k=0; k<operation.length; k++) {
                argument = operation[k];

                // Note that currently either Array or  Subroutine and Operation
                // are in use.
                if(!isInstance(argument, [_ValueToken, Array]))
            ...
            // 1 added, operation.length removed
            adjust = 1 - operation.length;
            if(adjust) {
     ...
     */
    _p.infixToPostfix = function (tokensArg) {
        var tokens = tokensArg.slice()
          , i, l, token
          , finished
        ...
        for(i=0,l=tokens.length;i<l;i++) {
            ...
            if(this._negateOperator
                ...
                token = this._operators[this._negateOperator[1]];
            }
            ...
            // it is a bracket
                ...
                finished = _toPostfix(current);
                // finished is either an Array/Operation, a _Valuetoken or another Array/Grouping
    ...
    _p.parse = function(string, selectorEngine) {
        ...
        tokens = _flatten([this.infixToPostfix(tokens)]);
