define([
  ...
  , 'Atem-Property-Language/_AbstractExpression'
  , './_ValueToken'
  , './NumberToken'
  , './StringToken'
  , './SelectorToken'
  ...
  , SelectorToken
    ...
    function PostfixExpression(postfixStack) {
        ...
        this.evaluate = stackCache[sig] || (stackCache[sig] = this._compile(postfixStack));
        // this.evaluate = this._compile();
    }

    var _p = PostfixExpression.prototype = Object.create(Parent.prototype);
    _p.constructor = PostfixExpression;
    ...
    _p._compile = function(_stack) {
        ...
        for(i=0,l=this._stack.length;i<l;i++) {
            if(_stack[i] instanceof _ValueToken)
                ...
                args = stack.splice(-_stack[i].consumes);
    ...
    _p.evaluate = function(getAPI) {
        ...
        for(;i<commands.length;i++) {
            if(commands[i] instanceof _ValueToken)
                ...
                args = stack.splice(-commands[i].consumes);
                // commands always return only one element currently
                returned = commands[i].execute(getAPI, args);
                stack.push( returned );
            }
        }
    ...
    _p.execute = function(getAPI) {
        errors.warn('The method `execute` is deprecated, use `evaluate` instead.');
        return this.evaluate(getAPI);
    };
